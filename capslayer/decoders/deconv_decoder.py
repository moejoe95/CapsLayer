...
class DeconvDecoderNet(object):
    ...
    def reconstruct_image(self):
        ...
        '''
        self.mask()
        with tf.compat.v1.variable_scope('Deconv_Decoder'):
            ...
            res1 = tf.image.resize(conv_rec1, (8, 8), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            conv_rec2 = tf.layers.conv2d(res1, name="conv2_rec", **conv_rec2_params)
            res2 = tf.image.resize(conv_rec2, (17, 17), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            conv_rec3 = tf.layers.conv2d(res2, name="conv3_rec", **conv_rec3_params)

            s = 16 // self.vec_shape[0]
            res3 = tf.image.resize(conv_rec3, (self.height *s, self.width *s), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
