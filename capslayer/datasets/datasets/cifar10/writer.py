...
def load_cifar10(split, path):
    ...
    if split == 'test':
        ...
        for i in range(1, 6):
            ...
            (images[(i - 1) * TEST_SIZE:i * TEST_SIZE, :, :, :],
             labels[(i - 1) * TEST_SIZE:i * TEST_SIZE]) = load_batch(fpath)

        idx = np.arange(len(images))
        np.random.seed(201808)
        np.random.shuffle(idx)

        images = images[idx[:TRAIN_SIZE]] if split == "train" else images[idx[TRAIN_SIZE:]]
        labels = labels[idx[:TRAIN_SIZE]] if split == "train" else labels[idx[TRAIN_SIZE:]]
